...
class TravellingSalesAgentProblem:
    ...
    def display_map(self,route=None):
        ...
        if not route:
            fig, ax = ox.plot_graph_route(self._street_graph,[self._origin], route_color="blue", bgcolor="gray", edge_color="white", edge_linewidth=2, node_size=0, show=False, close=False)
        else:
            full_route = self._get_full_route(location_order=route)
            fig, ax = ox.plot_graph_route(self._street_graph,full_route, route_color="blue", bgcolor="gray", edge_color="white", edge_linewidth=2, node_size=0, show=False, close=False)
